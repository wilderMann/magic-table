...
# library for multiple joysticks
joysticks = {}
player_id = {"player1": -1, "player2": -1}
...
def main():
    ...
    if not PI:
        ...
        while joystick_detected == False:
            show_message(device, "Waiting for controller...", fill="white", font=proportional(CP437_FONT),
                         scroll_delay=0.01)
            pygame.joystick.init()
            for x in range(pygame.joystick.get_count()):
                ...
                show_message(device, f"Joystick {joy.get_instance_id()} connected", fill="white",
                             ...
                             scroll_delay=0.01)
            joystick_detected = bool(joysticks)
        print("Gamepads found: " + str(pygame.joystick.get_count()))
        show_message(device, f"Joystick {pygame.joystick.get_count()} Joysticks found", fill="white",
                     font=proportional(CP437_FONT),
                     scroll_delay=0.01)
        print(joysticks)
...
def populatePlayers(joy):
    player = 0
    if player_id["player1"] == -1:
        player = "player1"
    elif player_id["player2"] == -1:
        player = "player2"
    else:
        print("Maximum players of 2 exceeded, exiting.")
        return
    player_id[player] = joy.get_instance_id()
    ...
    while True:  # main game loop
        ...
        for event in pygame.event.get():
            if event.type == pygame.JOYAXISMOTION:
                if event.instance_id is player_id["player1"]:
                    axis = event.axis
                    ...
                    if val == 0:  # On 0-Position end moment of Lower Player
                        movingLeftLower = False
                        movingRightLower = False
                elif event.instance_id is player_id["player2"]:
                    axis = event.axis
                    val = round(event.value)
                    if axis == 0 and val == 1:
...
def runSnakeGame():
    ...
    if os.path.isfile('/home/pi/hs_snake.p') == True:
        ...
        highscore = 0
    if PI:
        show_message(device, "Snake Highscore: " + str(highscore), fill="white", font=proportional(CP437_FONT),
                     scroll_delay=0.01)
    ...
    while True:  # main game loop
        ...
        if wormCoords[HEAD]['x'] == -1 or wormCoords[HEAD]['x'] == BOARDWIDTH or wormCoords[HEAD]['y'] == -1 or \
                wormCoords[HEAD]['y'] == BOARDHEIGHT:
            time.sleep(1.5)
            if score > highscore:
                highscore = score
                if PI:
                    pickle.dump(highscore, open("/home/pi/hs_snake.p", "wb"))
                    show_message(device, "New Highscore !!!", fill="white", font=proportional(CP437_FONT),
                                 scroll_delay=0.01)
            return  # game over
...
def runTetrisGame():
    ...
    if os.path.isfile('/home/pi/hs_tetris.p') == True:
        ...
        highscore = 0
    if PI:
        show_message(device, "Tetris Highscore: " + str(highscore), fill="white", font=proportional(CP437_FONT),
                     scroll_delay=0.01)
...
def shutdownScreen():
    if PI:
        ...
        show_message(device, "Press Select to shutdown!", fill="white", font=proportional(CP437_FONT),
                     scroll_delay=0.01)
    else:
            ...
            if event.type == pygame.JOYBUTTONDOWN or event.type == KEYDOWN:
                ...
                if (myevent != JKEY_SEL):
                    ...
                    if not PI:
                        ...
                        clearScreen()
                        updateScreen()
                        show_message(device, "Shutdown...", fill="white", font=proportional(CP437_FONT),
                                     scroll_delay=0.01)
